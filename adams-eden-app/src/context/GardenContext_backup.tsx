...
function summarizePlantingMode(periods: PlantPeriod[]) {
  ...
  if (hasIndoor && hasTransplant && hasOutdoor) return 'Indoor seed â†’ outdoor transplant';
...
export const GardenProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const plantDatabase = React.useMemo(() => buildPlantDatabase(springFrostDay), [springFrostDay]);
    ...
    try {
      ...
      try {
        ...
        locationSummary = buildLocationInfoFromRecord(record, {
          ...
          springFrostDay: climate.springFrostDay ?? DEFAULT_SPRING_FROST_DAY,
  ...
  const startTracking = (plant: Plant) => {
    setTrackedPlants(prev => {
      // Prevent duplicates by plant slug
      if (prev.some(tp => tp.slug === plant.slug)) {
        return prev;
      }
      const newTracked: TrackedPlant = {
        ...
        daysGrown: 0,
        plantedConfirmed: false,
        // Default watering settings (auto-populated from plant type)
        wateringFrequency: 'average', // Default to average, can be adjusted by user
        wateringIntervalDays: 4, // Every 4 days for average
        lastWatered: undefined, // No watering logged yet
        wateringReminderEnabled: true, // Reminders on by default
      };
      return [...prev, newTracked];
    });
  };

  const removeTrackedPlant = (trackingId: string) => {
    setTrackedPlants(prev => prev.filter(tp => tp.trackingId !== trackingId));
  };

  const updateTrackedPlantPlantedDate = (trackingId: string, plantedDateISO: string) => {
    ...
    setTrackedPlants(prev => prev.map(tp => (
      tp.trackingId === trackingId
        ? { ...tp, seedPlantedDate: normalized, daysGrown: 0, plantedConfirmed: true }
        : tp
    )));
  };

  const logWatering = (trackingId: string) => {
    setTrackedPlants(prev => prev.map(tp => (
      tp.trackingId === trackingId
        ? { ...tp, lastWatered: new Date().toISOString() }
        : tp
    )));
  };

  const updateWateringSettings = (
    trackingId: string,
    frequency: TrackedPlant['wateringFrequency'],
    intervalDays?: number
  ) => {
    setTrackedPlants(prev => prev.map(tp => {
      if (tp.trackingId !== trackingId) return tp;
      ...
      if (newInterval === undefined) {
        switch (frequency) {
          ...
          case 'minimum':
            ...
            break;
          case 'custom':
            newInterval = tp.wateringIntervalDays; // Keep existing
            break;
        }
      }
      
      return { ...tp, wateringFrequency: frequency, wateringIntervalDays: newInterval };
    }));
  };

  const toggleWateringReminder = (trackingId: string, enabled: boolean) => {
    setTrackedPlants(prev => prev.map(tp => (
      tp.trackingId === trackingId
        ? { ...tp, wateringReminderEnabled: enabled }
        : tp
    )));
  };
